...
const server = http.createServer((req, res) => {
    ...
    console.log(`[RESOLVED IP] ${visitorIp}`);
...
function sendBark(ip, location, isp, page, time, referrer) {
    const title = encodeURIComponent('📍 博客访客 (Debug)');
    ...
    const barkReq = client.get(barkFullUrl, (barkRes) => {
        barkRes.on('data', () => { });
        barkRes.on('end', () => { console.log(`[${new Date().toLocaleString('zh-CN')}] ✅ ${ip} | ${location}`); });
    });
    barkReq.on('error', (err) => { console.error(`Bark Error:`, err.message); });
    barkReq.end();
}

server.listen(PORT, '127.0.0.1', () => {
    console.log(`[Debug Relay] Running on port ${PORT}`);
});
